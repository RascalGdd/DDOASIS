...
class BaseModel(nn.Module,ABC):
    def __init__(self, opt):
        ...
        if opt.phase == "train":
            ...
            self.netDs = discriminators.OASIS_Discriminator(opt)
            if opt.netDu == 'wavelet':
                self.netDu = discriminators.WaveletDiscriminator(opt)
            else :
                self.netDu = discriminators.OASIS_Discriminator(opt)
            self.criterionGAN = losses.GANLoss("nonsaturating")
            self.featmatch = torch.nn.L1Loss()
    ...
    def load_checkpoints(self):
        if self.opt.phase == "test":
            ...
            self.netDs.load_state_dict(torch.load(path + "Ds.pth"))
    ...
    def print_parameter_count(self):
        if self.opt.phase == "train":
            networks = [self.netG, self.netD, self.netDu, self.netDs]
        else:
            networks = [self.netG]
    ...
    def init_networks(self):
        ...
        if self.opt.phase == "train":
            networks = [self.netG, self.netD, self.netDs]
        else:
            networks = [self.netG]
        for net in networks:
...
class SemiSupervisedModel(BaseModel):
    ...
    def forward(self, image, label, mode, losses_computer):
        ...
        if mode == "losses_G_unsupervised":
            ...
            return loss_G, [loss_G_adv, loss_G_GAN, loss_G_edge]


        if mode == "losses_G_supervised":
            loss_G = 0
            fake = self.netG(label, edges=edges)
            output_Ds = self.netDs(fake)
            loss_G_adv = losses_computer.loss(output_Ds, label, for_real=True)
            loss_G += loss_G_adv

            loss_G_feat = 0
            fake_features = self.netDs(fake, for_features=True)
            real_features = self.netDs(image, for_features=True)
            for real_feat,fake_feat in zip(real_features,fake_features):
                loss_G_feat += self.featmatch(real_feat.detach(),fake_feat)/len(fake_features)*self.opt.lambda_feat
            ...
            # loss_G_feat = None #modify
            return loss_G,[loss_G_feat, loss_G_adv, loss_G_vgg]
        ...
        if mode == "losses_Du_unsupervised":
            loss_Du = 0
            with torch.no_grad():
                fake = self.netG(label,edges = edges)
            output_Du_fake = self.netDu(fake)
        ...
        if mode == "losses_Ds_supervised":
            loss_Ds = 0
            with torch.no_grad():
                fake = self.netG(label,edges=edges)
            output_Ds_fake = self.netDs(fake)
            loss_Ds_fake = losses_computer.loss(output_Ds_fake, label, for_real=False)
            loss_Ds += loss_Ds_fake
            output_Ds_real = self.netDs(image)
            loss_Ds_real = losses_computer.loss(output_Ds_real, label, for_real=True)
            loss_Ds += loss_Ds_real
            if not self.opt.no_labelmix:
                mixed_inp, mask = generate_labelmix(label, fake, image)
                output_Ds_mixed = self.netDs(mixed_inp)
                loss_Ds_lm = self.opt.lambda_labelmix * losses_computer.loss_labelmix(mask, output_Ds_mixed,
                                                                                     output_Ds_fake,
                                                                                     output_Ds_real)
                loss_Ds += loss_Ds_lm
            else:
                loss_Ds_lm = None
            return loss_Ds, [loss_Ds_fake, loss_Ds_real, loss_Ds_lm]
