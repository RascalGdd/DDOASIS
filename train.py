...
if opt.model_supervision == 0 :
    ...
    model = models.Semi_supervised_model(opt)
...
for epoch in range(start_epoch, opt.num_epochs):
    for i, data_i in enumerate(dataloader):
        ...
        if opt.model_supervision == 0 :
            ...
            visualizer_losses(cur_iter, losses_G_list+losses_D_list+losses_Du_list+losses_reg_list)
        else:

            # --- supervised generator update ---#    
            ...
            #--- supervised discriminator update ---#
            model.module.netD.zero_grad()
            p_loss_D, p_losses_D_list = model(p_image, p_label, "losses_D_supervised", losses_computer)
            p_loss_D, p_losses_D_list = p_loss_D.mean(), [loss.mean() if loss is not None else None for loss in p_losses_D_list]
            p_loss_D.backward()
            optimizerD.step()

             # --- unsupervised generator update ---#       
            model.module.netG.zero_grad()
            ...
            if cur_iter % opt.freq_fid == 0 and cur_iter > 0:
                ...
                if is_best:
                    utils.save_networks(opt, cur_iter, model, best=True)
                _ = miou_computer.update(model,cur_iter)
            visualizer_losses(cur_iter, losses_G_list+p_losses_G_list+losses_D_list+p_losses_D_list+losses_Du_list+losses_reg_list)
