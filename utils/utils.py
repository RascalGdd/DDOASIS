...
class losses_saver():
    def __init__(self, opt):
        ...
        self.name_list = ["S_feat", "S_Ds(G)", "S_vgg", "S_Ds(fake)", "S_Ds(real)", "S_Ds(lm)", "U_D(G)", "U_Du(G)", "U_edge","U_Du(fake)", "U_Du(real)", "U_D", "regularize"]
        #self.name_list = ["S_feat", "S_Ds(G)", "S_vgg", "S_Ds(fake)", "S_Ds(real)", "S_Ds(lm)"]
        self.opt = opt
    ...
    def __call__(self, epoch, losses):
        ...
        if epoch % self.freq_smooth_loss == 0:#self.freq_smooth_loss-1:
            for i, loss in enumerate(losses):
        ...
        if epoch % self.freq_save_loss == 0:#self.freq_save_loss-1:
            self.plot_losses()
...
def save_networks(opt, cur_iter, model, latest=False, best=False):
    ...
    if latest:
        ...
        torch.save(model.module.netDs.state_dict(), path + '/%s_Ds.pth' % ("latest"))
        if not opt.no_EMA:
        ...
        torch.save(model.module.netDs.state_dict(), path + '/%s_Ds.pth' % ("best"))
        if not opt.no_EMA:
        ...
        torch.save(model.module.netDs.state_dict(), path + '/%d_Ds.pth' % (cur_iter))
        if not opt.no_EMA:
